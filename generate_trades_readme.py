...
def trades_to_markdown(trades: dict) -> str:
    def format_item(item_id: str, qty: int) -> str:
        """
        Convert 'minecraft:foo_bar' + quantity into a readable string.
        Always include the quantity, even if it is 1.
        """
    ...
    for category in trades.values():
        max_qty = category["maximum_quantity"]
        lines.append(f"- Up to {max_qty} of the following:")

        for trade in category["trades"]:
            buy = format_item(trade.buy_item, trade.buy_quantity)
            sell = format_item(trade.sell_item, trade.sell_quantity)

            lines.append(
                f"  - {buy} for {sell} ({trade.max_uses} max uses)"
            )
...
def main() -> None:
    ...
    logger.debug(f'{trade_sections=}')
    text = trades_to_markdown(trade_sections)
    print(text)
